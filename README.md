...
The hosted service vendors `protocol/v0.2.5` (manifest, payload and key
...
|---|---|---|---|---|
| `@airprompter/agent-core` | 0.1.0 | 0.2.5 | `canonical-json.json`, `manifest-verify.json`, `assignment.json`, `ramp.json`, `workflow-steps.json`, `checks.json` | green |
| `@airprompter/agent-sync` | 0.1.0 | 0.2.5 | `examples/store.v1.json`, `examples/store.v2.json`, `examples/refused/*` | green |
| `@airprompter/agent-runtime` | 0.1.0 | 0.2.5 | `ramp.json (walked by `ReleaseResolver` over a bundle release)` | green |
| `@airprompter/agent-telemetry` | 0.1.0 | 0.2.5 | `spool.json`, `feedback.json` | green |
| `@airprompter/agent-sdk` | 0.1.0 | 0.2.5 | `examples/heartbeat.*.json`, `examples/edge-pointer.json` | green |

Generated by `sdk-typescript/scripts/compat-table.mjs` from the conformance run on protocol `0.2.5` (Node 24). One version across the five packages; `@airprompter/agent-sdk` exact-pins its siblings.
<!-- compat-table:end -->
