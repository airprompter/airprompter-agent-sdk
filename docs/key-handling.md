...
## Which package holds which key (S10)

| Key | Package |
|---|---|
| The pinned root, the signing keys it names, the countersign root | `@airprompter/agent-core` (`trustedRootFromPinnedKey`, `verifyRootMetadata`, `verifyManifest`) — verification only, no key material is generated here |
| The store DEK and the KEK providers (file key, OS keystore, KMS, Vault, custom) | `@airprompter/agent-sync` (`SlotStore`, `keyProvider`) |
| The distribution key (`.apbundle` HPKE) | opened by `@airprompter/agent-core` (`openBundle`); generated and stored by the CLI (`airprompter keygen --purpose distribution`) |
| The customer's countersign key | generated by the CLI (`keygen --purpose countersign`); the public half is what core verifies against |
| The Agent key and the telemetry grant | `@airprompter/agent-core` (the control-plane client) · `@airprompter/agent-telemetry` (the uploader's AirPrompter sink); never in a bundle, never on argv |
| The apply-policy pin (not a key, but the host's authority) | `store.json` in `@airprompter/agent-sync` |
