...
- `slots[].variables[].default` (optional; a string, ≤ 4096 characters): the text rendered when neither the call site nor a source the application registered (`docs/variables.md`) supplies a value. Only on an optional `operator` variable — the control plane refuses a default on a required or an end-user variable at seal time, the schema says so (`if`/`then`), and a runtime ignores one there. Applied by `renderTemplate` / `render_template` and by the hosted run route, so golden, client and hosted renders agree. In the digest input when present.
- `slots[].variables[].source` (optional; `caller` | `runtime`): who the author expects to fill the variable — the code that renders, or a source the application registers. A hint reviewers and the seal read; a runtime does not act on it. In the digest input when present.
- The heartbeat's `catalog.variables` (optional; names only, never values): the variable names this instance can fill from the application's own sources. The seal warns `variable_uncovered` when a pin declares a `source: runtime` variable that no live instance of the environment reports, before the promotion rather than after.
- `canonical-json.md`: `variables[{ name, required, trust, default (only when present), source (only when present) }]` — a slot without them keeps its digest; a JSON `null` is unset, as for an inference key. Reference, TypeScript and Python agree; a trust vector seals both. Additive: a 0.3.3 runtime verifies a 0.3.4 manifest and renders a defaulted variable empty. Examples and every vector regenerated at 0.3.4.
- The hosted slots catalogue (`openapi.yaml`) carries `default` and `source` on each declared variable. `airprompter dev` front matter: `name=default`, `name~` (source: runtime), `name~=default`.

### 0.3.3 — the manifest answer names the edge pointer; a puller polls the CDN (SDKs 0.2.8, 0.2.9)
- `GET …/manifest` carries `x-agent-edge-pointer-url` on `200` and `304`: the target's `generation.json` behind the CDN, the same URL the heartbeat has always returned as `edgePointerUrl`. A puller — which never heartbeats — learns it from its first pull. Additive; a deployment without an edge omits it.
- SDKs 0.2.8: `pullBundle` / `pull_bundle` read the pointer first when the caller hands back `edge` (the URL and two ETags) from the last result — a 304, or a generation the caller already holds, is `{ status: "unchanged", via: "pointer" }` and the origin is never called; only a moved pointer reaches the API, and that read is conditional (`If-None-Match`), so an unmoved origin is `unchanged` via `origin` instead of the old `unavailable http_304`. `skipPointer` reads the origin at once (a nudge, an operator). `nextPullDelayMs` / `next_pull_delay_ms` doubles the interval while nothing changes, to a cap (five minutes by default), and snaps back on any change, refusal or outage. Steady state per puller: one CDN 304 per interval, zero API requests. 0.2.9 (the review of 0.2.8) adds three rules that keep the unsigned pointer honest: it is believed for `maxPointerAgeMs` (one hour) after the origin last answered, then the origin is read once (a pinned or unwritten pointer can hide a promotion at most that long); nothing in the edge state advances past an answer the origin did not confirm; a CDN outage or a malformed pointer falls through to the origin. `docs/change-notification.md` sets out the optional nudge on a channel the customer owns.
