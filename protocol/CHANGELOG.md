...
- SDKs 0.2.8: `pullBundle` / `pull_bundle` read the pointer first when the caller hands back `edge` (the URL and two ETags) from the last result — a 304, or a generation the caller already holds, is `{ status: "unchanged", via: "pointer" }` and the origin is never called; only a moved pointer reaches the API, and that read is conditional (`If-None-Match`), so an unmoved origin is `unchanged` via `origin` instead of the old `unavailable http_304`. `skipPointer` reads the origin at once (a nudge, an operator). `nextPullDelayMs` / `next_pull_delay_ms` doubles the interval while nothing changes, to a cap (five minutes by default), and snaps back on any change, refusal or outage. Steady state per puller: one CDN 304 per interval, zero API requests. `docs/change-notification.md` sets out the optional nudge on a channel the customer owns.
