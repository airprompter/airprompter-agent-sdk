...
- The heartbeat's `catalog.variables` (optional; names only, never values; ≤ 256, unique): the variable names this instance can fill from the application's own sources. A runtime sends it only once the release it serves was sealed at protocol ≥ 0.3.4 (`protocolAtLeast` / `protocol_at_least`): a service at 0.3.3 refuses the whole heartbeat over an unknown key, and a refused heartbeat is worse than an unreported name. Past that gate it is always sent (SDKs 0.2.13): an empty list is a report — "I fill nothing" — and the service counts only reporting instances when it says "filled on X of Y". With the service's 0.3.4 release the seal warns when a `source: runtime` variable is filled by no live instance of the environment, before the promotion rather than after.
- `canonical-json.md`: `variables[{ name, required, trust, default (only when present), source (only when present) }]` — a slot without them keeps its digest. Reference, TypeScript and Python agree; a trust vector seals both. Additive: a 0.3.3 runtime verifies a 0.3.4 manifest and renders a defaulted variable empty. Examples and every vector regenerated at 0.3.4.
- The hosted slots catalogue (`openapi.yaml`) carries `default` and `source` on each declared variable. `airprompter dev` front matter: `name=default`, `name~` (source: runtime), `name!~` / `name?~`, `name~=default`; the name is checked against the grammar, and an empty default is refused.
