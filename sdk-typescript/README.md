...
| `@airprompter/agent-telemetry` | Your own instrumentation writes the content-free spool (`SpoolWriter`, `DirectorySink` / `MemorySink`) and ships it under a grant (`SpoolUploader`). | core |
| `@airprompter/agent-sdk` | One install with today's `AirPrompterAgent`: the facade over the four, every public name re-exported. | all four |
